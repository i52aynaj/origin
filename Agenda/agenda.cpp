...
int Agenda::buscarAlumno(string DNI){
	for(int i=0; i<getListaAlumnos().size();i++){
		if(getListaAlumno().at(i).getDNI()==DNI){
			return i;
		}
	}
...
int cont=0;
	for(int i=0; i<getListaAlumnos().size();i++){
		if((getListaAlumno().at(i).getApellidos()==apellidos)and(getListaAlumno().at(i).getEquipo()==equipo)){
			cont++;
			aux=i;
		}
	}
...
bool Agenda::validacionDatos(Alumno alumno){
	...
	int cont=0;
	for(int i=0; i<getListaAlumnos().size();i++){
		if((getListaAlumno().at(i).getDNI()==alumno.getDNI())or(getListaAlumno().at(i).getEmail()==alumno.getEmail())){
			return false;
		}
	}
	return true;
